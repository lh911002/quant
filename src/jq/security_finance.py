    ...
    for stock_index in range(len(stocks)):
        ...
        for index in range(len(df_finance)):
            ...
            if item.end_date.month == 12:  # 年报数据
                ...
                if profit_percent > 2.0:  # 过滤业绩波动巨大的公司
                    flag = True
                    break
                elif profit_percent <= -2.00:
                    flag = True
                    break
                elif profit_percent <= 0.00:
                    continue
                else:
                    count = count + 1
        if years_count <= 1 or last_year_profit <= 0 or flag:
            continue

        if first_year_profit > 0:
            change_one_year = math.pow(last_year_profit/first_year_profit, 1/years_count)-1 #年复合增长
